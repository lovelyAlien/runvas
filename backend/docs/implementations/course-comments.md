...
| 파일 | 종류 | 내용 |
|------|------|------|
| `db/migration/V7__create_course_comments.sql` | 신규 | `course_comments` 테이블, `(course_id, created_at)` 인덱스 |
...
## 추가 변경: 대댓글 단계 제한 제거 (다단계 스레드)

기존에는 "최상위 댓글 → 대댓글" 2단계까지만 허용하고, 대댓글에 다시 답글을 달면
`400 VALIDATION_ERROR`를 반환했습니다. 이번에 그 제한을 제거해 대댓글에도 다시 대댓글을 달 수
있도록(단계 제한 없음) 구조를 변경했습니다. `docs/api-contract.md`, `docs/data-model.md`의
관련 설명도 함께 수정했습니다.

- `CourseCommentService.validateParentComment`에서 "부모가 이미 대댓글이면 거부"하던 분기를
  제거했습니다. `parentCommentId`는 이제 같은 코스에 속한 어떤 댓글이든 가리킬 수 있습니다.
- `GET /comments/{commentId}/replies`는 원래부터 `parentCommentId`로 단순 필터링하는 쿼리라
  구조 변경이 필요 없었습니다. 다만 각 대댓글의 `replyCount`가 항상 `0`으로 하드코딩되어 있던
  버그를 함께 고쳤습니다(대댓글도 자신의 대댓글 수를 가질 수 있으므로).
- `update()`도 마찬가지로 "최상위 댓글일 때만 replyCount를 계산"하던 조건을 제거하고 항상
  실제 대댓글 수를 계산하도록 했습니다.
- `delete()`는 DB의 `ON DELETE CASCADE`(자기 참조 FK)가 몇 단계든 행 삭제를 자동으로
  전파해주지만, 첨부 이미지 파일은 DB가 지워주지 않습니다. 기존에는 삭제 대상이 최상위 댓글일
  때만 "직계 대댓글"의 이미지를 지웠는데, 다단계 구조에서는 이것으로 부족해서
  `collectDescendants`로 모든 하위 댓글(대댓글의 대댓글 포함)을 재귀적으로 수집해 이미지를
  먼저 지우도록 변경했습니다.

### 테스트 변경

- `replyToReplyIsRejected`(2단계 초과 시 400 검증)를 제거하고, 대신
  `replyToReplySucceedsAndNestsCorrectly`로 대체해 대댓글의 대댓글이 성공하고
  `parentCommentId`/`replyCount`가 올바르게 연결되는지 검증합니다.
- `deletingParentCommentCascadesToReplies`를 `deletingParentCommentCascadesToNestedReplies`로
  확장해 2단계 중첩(대댓글의 대댓글)까지 삭제 시 함께 제거되는지 확인합니다.
- `./gradlew test --tests CourseCommentControllerTest` → 10/10 통과.

## 추가 변경: 댓글 이미지 첨부 기능 제거

"댓글에 사진을 올릴 수 있는 기능"을 완전히 제거해달라는 요청에 따라, 댓글 도메인에서 이미지
관련 코드를 전부 걷어냈습니다. 제거 전 `ImageStorageService`/`UploadWebConfig`/
`runvas.upload.*` 설정이 이 기능 외에 다른 곳에서도 쓰이는지 먼저 확인했고(코스 썸네일, 프로필
사진 등 다른 업로드 기능은 존재하지 않음 — `profileImageUrl`은 카카오가 제공하는 외부 URL을
그대로 저장하는 필드라 이 스토리지 파이프라인과 무관), 전부 댓글 기능 전용이라는 것을 확인한
뒤 관련 클래스를 통째로 삭제했습니다.

- `CourseComment.java`: `imageUrl` 컬럼/필드와 그 값을 받는 생성자 오버로드를 제거했습니다.
- `CourseCommentService.java`: `ImageStorageService` 의존성과 `create`/`update`/`delete`의
  이미지 저장·교체·삭제 로직을 모두 제거했습니다. `delete()`는 이제 DB의
  `ON DELETE CASCADE`(자기 참조 FK)에만 의존하므로, 이미지 정리를 위해 추가했던
  `collectDescendants` 재귀 헬퍼도 함께 삭제했습니다(하위 댓글 행 자체는 여전히 CASCADE로
  삭제됩니다. 이 헬퍼는 이미지 파일 정리 전용이었습니다).
- `CourseCommentController.java`: `create`/`update` 엔드포인트에서 `MultipartFile image`,
  `Boolean removeImage` 파라미터를 제거했습니다. 요청은 여전히 `multipart/form-data`이지만
  파일 파트 없이 텍스트 필드(`body`, `parentCommentId`)만 주고받습니다.
- `dto/CourseCommentResponse.java`: `imageUrl` 필드를 제거했습니다.
- `storage/ImageStorageService.java`, `config/UploadWebConfig.java`를 파일째로 삭제했습니다
  (댓글 기능에만 쓰이던 로컬 디스크 저장/`/uploads/**` 정적 서빙 코드).
- `application.yml`: `runvas.upload.dir`/`runvas.upload.base-url` 설정과, 이미지 첨부 때문에
  올려뒀던 `spring.servlet.multipart.max-file-size`/`max-request-size`(5MB/6MB) 제한을
  제거했습니다. 백엔드에 남은 유일한 파일 업로드 경로가 없어졌기 때문입니다.
- `db/migration/V9__drop_image_url_from_course_comments.sql`(신규): `course_comments.image_url`
  컬럼을 drop하는 마이그레이션을 추가했습니다. 기존 마이그레이션 파일(V7)은 수정하지 않고
  새 마이그레이션으로 스키마 변경을 표현했습니다.

### 테스트 변경 (이미지 제거)

- `createCommentWithInvalidImageExtensionReturns400`, `createCommentWithOversizedImageReturns400`
  테스트를 삭제했습니다(더 이상 검증할 대상이 없음).
- `createCommentOnPublicCourseSucceedsWithoutImage`를 `createCommentOnPublicCourseSucceeds`로
  이름을 바꾸고 `imageUrl` 관련 단언을 제거했습니다.
- 테스트의 `@TempDir uploadDir`, `runvas.upload.*` 동적 프로퍼티 등록, 업로드 디렉터리 정리용
  `@AfterAll cleanupUploadDir`를 모두 제거했습니다.
- `./gradlew test` → 전체 테스트 통과(0 failures, 0 errors).
