...
- **2026-09 App Store review, Guideline 2.1(a) (App Completeness)**: Apple's reviewer reported the app becoming unresponsive after tapping "Sign in with Apple" on iPad (iPadOS 27.0). Root cause was the same class of collision, but in `mobile/src/contexts/AuthContext.tsx` rather than `App.tsx`: `kakaoLogin`/`appleLogin` call `setIsLoginModalVisible(false)` and `setIsTermsModalVisible(true)` in the same batch when `termsAgreedAt` is unset (true for any fresh install/demo account, which is exactly what a reviewer's device is), and `agreeToTerms`/`cancelTermsAgreement` do the reverse (`setIsTermsModalVisible(false)` + `setIsLoginModalVisible(true)`) when the terms modal closes. Both `LoginPromptModal` and `TermsAgreementModal` are separate RN `Modal` components rendered as siblings in `mobile/src/components/LoginPromptModal.tsx`, so the same silently-dropped-`presentViewController` failure applies. Fixed on branch `fix/apple-signin-terms-modal-race` by deferring the second modal's `setState` by 350ms at all four call sites, matching the pattern in `mobile/App.tsx`.
