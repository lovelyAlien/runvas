...
| `/admin/reports` | 신고 목록. 기본 필터 `PENDING`, `status`/`targetType`로 필터링, 페이지네이션(20건). 각 행에 신고 대상 콘텐츠 미리보기, 신고자 ID, 사유, 신고 시각. `PENDING` 신고에 한해 "삭제"(콘텐츠 하드 삭제 + 신고 `RESOLVED`)/"삭제+정지"(콘텐츠 하드 삭제 + 작성자 계정 정지 + 신고 `RESOLVED`)/"기각"(신고만 `DISMISSED`) 처리 가능 |

### 신고 삭제 + 작성자 정지

`PENDING` 신고 목록에서 "삭제+정지" 버튼을 누르면, 해당 신고 대상 콘텐츠를 삭제하는 것과
동시에 그 콘텐츠를 작성한 이용자의 계정을 정지시킨다. 정지된 계정은 이후 카카오/Apple
로그인 시 `403 FORBIDDEN`으로 거부된다. 이미 발급되어 있던 토큰도 정지 시점 이후의 다음
요청부터 즉시 무효화되며(기기·세션 무관), 토큰 만료 전이라도 계속 사용할 수 없다. 단순
삭제("삭제" 버튼)는 콘텐츠만 지우고 계정은 정지시키지 않는다 — 위반 정도가 가벼운 경우
"삭제"만, 반복·심각한 위반은 "삭제+정지"를 사용한다.

## 명시적 제외 범위

- 신고 처리(콘텐츠 삭제/삭제+정지/기각)를 제외한 나머지 화면(회원/코스/게시글 목록)은
  여전히 조회 전용입니다. 신고 처리 흐름 밖에서 임의로 회원을 정지시키는 별도 UI는 없습니다.
