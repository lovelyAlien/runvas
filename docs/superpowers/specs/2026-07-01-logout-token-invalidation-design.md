...
- 백엔드 호출이 **성공한 경우에만** `SecureStore`(`TOKEN_KEY`, `USER_KEY`) 삭제 + `user`/`accessToken`
  state를 초기화한다.
- 실패 시 로컬 상태를 그대로 유지한다. `loginError`와 대칭되는 `logoutError` state를 추가해 호출부가
  에러 메시지를 표시할 수 있게 한다.
...
- 실패 시 `Alert.alert('오류', logoutError)`로 안내하고 화면에 남는다 (로그인 상태 유지).
...
| 모바일 로그아웃 API 호출 실패(네트워크/서버 오류) | 로컬 상태 유지, `logoutError` 표시, 사용자는 재시도 가능 |
