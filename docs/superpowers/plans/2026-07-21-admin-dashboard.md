...
- [ ] **Step 5: 기존 SecurityConfig에 순서 명시**
...
체인 빌더 시작부는 그대로 둔다 (`@Order`만으로 충분하다 — Spring Security는 등록된
`SecurityFilterChain` 중 순서대로 첫 번째로 매칭되는 체인을 쓰므로, `adminSecurityFilterChain`이
`@Order(1)` + `securityMatcher("/admin/**")`로 `/admin/**`를 먼저 가로채고, 나머지 모든 요청
(`/api/**` 포함)은 매처가 없는 이 체인이 `@Order(2)`로 그대로 받는다. 여기에
`.securityMatcher("/api/**")`를 추가로 넣으면 `com.runvas.global.security.SecurityConfigTest`가
쓰는 `@WebMvcTest` 슬라이스 테스트—`SecurityConfig` 체인 하나만 등록하고 `/api/**` 밖의
`/test/protected`를 호출—가 어떤 체인에도 안 걸려 인증 없이 통과해버려 500(NPE)으로 깨진다.
프로덕션 라우팅에는 차이가 없으므로 이 변경은 하지 않는다):
...
Expected: `BUILD SUCCESSFUL` — `SecurityConfig`에는 `@Order(2)`만 추가했고 매칭 범위는 그대로이므로
기존 JWT 인증 동작(및 `@WebMvcTest` 슬라이스 테스트)이 변경 없이 그대로 유지된다.
